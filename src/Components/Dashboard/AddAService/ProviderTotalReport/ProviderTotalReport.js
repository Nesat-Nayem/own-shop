...
const ProviderTotalReport = () => {
  ...
  useEffect(()=>{
    fetch(`https://energetic-pear-threads.cyclic.app/api/orders/provideremailorder/${user.email}`)
    .then(res=>res.json())
  ...
  useEffect(()=>{
    fetch(`https://energetic-pear-threads.cyclic.app/api/products/providerservices/${user.email}`)
    .then(res=>res.json())
